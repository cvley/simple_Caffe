...
class Layer {
 public:
   ...
   */
  void SetUp(const std::vector<Blob<Dtype>*>& bottom, std::vector<Blob<Dtype>*>* top) {
 ...
 protected:
   ...
   */
  inline void SetLossWeights(std::vector<Blob<Dtype>*>* top) {
...
// functions.
template <typename Dtype>
inline Dtype Layer<Dtype>::Forward(const std::vector<Blob<Dtype>*>& bottom,
    std::vector<Blob<Dtype>*>* top) {
  Dtype loss = 0;
  switch (Caffe::mode()) {
  case Caffe::CPU:
...
template <typename Dtype>
inline void Layer<Dtype>::Backward(const std::vector<Blob<Dtype>*>& top,
    const std::vector<bool>& propagate_down,
    std::vector<Blob<Dtype>*>* bottom) {
  switch (Caffe::mode()) {
  case Caffe::CPU:
